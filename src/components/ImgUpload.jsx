...
const ImgUpload = () => {
  ...
  const {inputL, img, inputImg, btnClearImg, clearImg, loading, chargeImg, saveImg, btnSaveImg, deleteImg, inputImgRef, images, ServiceReportId} = useContext(ImgDataContext)
      ...
      <section className="flex flex-col items-center gap-3">
        ...
        <label className={`${inputL && 'hidden'} cursor-pointer bg-blue-700 text-white hover:bg-blue-600 p-2 rounded-[12px] `}>
          ...
          <input
            ...
            onChange={inputImg}
            type="file"
            ref={inputImgRef} // Asignar la referencia al input
          />
        </label>
        {/* Div contenedor de img */}
        <div className={`${img && 'hidden'} relative w-ful flex justify-center`}>
        {btnClearImg && (
          <button 
              onClick={clearImg} 
        ...
        <div>
          <button onClick={saveImg} className={` ${btnSaveImg && 'hidden'} bg-blue-700 text-white hover:bg-blue-600 p-2 rounded-[12px]`}>
            Guardar
          </button>
        </div>
      </section>
        ...
        {images
          ...
          .map(img => ( // Mapea los elementos filtrados para renderizarlos
            <div className="relative " key={img.id}>
              <button className="absolute rounded-full w-6 h-6 bg-black/30 text-white/50 hover:bg-white/50 hover:text-black/50" onClick={(e) => deleteImg(img.id, e)}>
                X
              </button>
